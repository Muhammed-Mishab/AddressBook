...
Date : 10-10-2025
Description : Address book application to add, edit, delete, search and list contacts functions
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int name_search(struct AddressBook *addressBook) // sub function for name search handles multiple name cases
{
	char temp_name[50];
	printf("\nEnter the name: ");
	scanf(" %[^\n]",temp_name);
	...
	else if(count > 1) // Multiple contact
	{
		...
		printf("\n%d contacts are found for %s\nFrom these choose the contact: ", count, temp_name);
...
int phone_search(struct AddressBook * addressBook) // sub function for searching phone
{
